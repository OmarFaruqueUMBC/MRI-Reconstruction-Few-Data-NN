...
from analyze_loader import load_image_data, get_image_file_paths
...
def load_and_subsample(raw_img_path, substep, low_freq_percent):
    ...
    if num_slices > NUM_SAMPLE_SLICES:
        relevant_idx_low = int( (num_slices - NUM_SAMPLE_SLICES) // 2 )
...
def functionTrain(image_path, checkpoints_dir, num_imgs, num_gpus, num_epochs):
    training_error ='mse'
    lf_percent = float('0.04')
    substep = int('4')
    batch_size = int('256')
    ...
    x_train, y_train = load_and_subsample_images(
        ...
        num_imgs=num_imgs,
        substep=substep,
        low_freq_percent=lf_percent)

    if len(x_train) > num_imgs:
